...
public class ClickHandler2 : MonoBehaviour, IPointerClickHandler
{
	...
	private static void StaticLog(string message, params object[] args)
	{
		...
		m_StaticLogText.text += System.Environment.NewLine;
		m_StaticLogText.text += System.Environment.NewLine;
	}
	...
	{
		StaticLog("TestStaticString method. Dynamic PointerEventData param:{0}, string param:{1}", eventData, stringParam);
	}

	public static void TestStaticString(PointerEventData eventData, string stringParam1, string stringParam2)
	{
		StaticLog("TestStaticString method. Dynamic PointerEventData param:{0}, string param1:{1}, string param2:{2}", eventData, stringParam1, stringParam2);
	}
	...
	{
		StaticLog("TestStaticEnum method. Dynamic PointerEventData param:{0}, enum param1:{1} - enum param2:{2} - enum param3:{3}", eventData, enumExample1, enumExample2, enumExample3);
	}

	public static void TestStaticArrayString(PointerEventData eventData, string[] array)
	{
		StaticLog("TestStaticArrayString method. Dynamic PointerEventData param:{0}, array length:{1}, values:{2}", eventData, array.Length, string.Join(",", array.Select(x => x.ToString()).ToArray()));
	}

	public static void TestStaticListString(PointerEventData eventData, List<string> list)
	{
		StaticLog("TestStaticListString method. Dynamic PointerEventData param:{0}, list count:{1}, values:{2}", eventData, list.Count, string.Join(",", list.Select(x => x.ToString()).ToArray()));
	}
