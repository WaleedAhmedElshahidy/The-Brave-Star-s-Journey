	...
	public class UnityEvent2Drawer : PropertyDrawer
	{
			...
			using (new EditorGUI.DisabledScope(listenerTarget.objectReferenceValue == null))
			{
				EditorGUI.BeginProperty(functionRect, GUIContent.none, methodName);
				{
					...
					else
					{
						...
						else if (!IsListenerValid(listenerTarget.objectReferenceValue, methodName.stringValue, desiredTypes, isStatic.boolValue))
						{
						...
						{
							buttonLabel.Append(isStatic.boolValue ? "static " : string.Empty);
		...
		private void BuildArgumentsField(SerializedProperty listener, Rect argNameRect, Rect argRect, UnityEventBase2 dummyEvent)
		{
			...
			if (listenerTarget.objectReferenceValue != null)
				method = GetMethodInfo(listenerTarget, methodName, arguments, isStatic);
				...
				else
				{
					...
					for (int i = 0; i < arguments.arraySize; i++)
					{
						...
						if (methodSliderAttr == null || methodIntSliderAttr == null)
						{
							...
							{
								if (methodSliderAttr == null && attr is SliderAttribute sliderAttr)
									paramSliderAttr = sliderAttr;

								if (methodIntSliderAttr == null && attr is IntSliderAttribute intSliderAttr)
									paramIntSliderAttr = intSliderAttr;
		...
		private void GeneratePopUpForType(GenericMenu menu, Object target, bool useFullTargetName, SerializedProperty listener, Type[] delegateArgumentsTypes)
		{
			...
			{
				GetMethodsForTargetAndMode(target, methods, true, delegateArgumentsTypes: delegateArgumentsTypes);
				if (methods.Count > 0)
				{
		...
		private void AddMethodsToMenu(GenericMenu menu, SerializedProperty listener, List<ValidMethodMap> methods, string targetName, Type[] delegateArgumentsTypes = null)
		{
			...
			IEnumerable<ValidMethodMap> orderedMethods = methods.OrderBy(e => e.methodInfo.Name.StartsWith("set_") ? 0 : 1).ThenBy(e => e.methodInfo.IsStatic ? 0 : 1).ThenBy(e => e.methodInfo.Name);
		...
		private void AddFunctionsForScript(GenericMenu menu, SerializedProperty listener, ValidMethodMap method, string targetName, Type[] delegateArgumentsTypes)
		{
			...
			var isStatic = listener.FindPropertyRelative(kIsStaticPath).boolValue;
			var modes = listener.FindPropertyRelative(kModesPath);
		...
		private string GetFormattedMethodName(string targetName, MethodInfo method, string args, bool dynamic, Type[] delegateArgumentsTypes)
		{
			...
			{
				if (method.Name.StartsWith("set_"))
					return string.Format("{0}/{1}{2} ({3})", targetName, method.IsStatic ? "static " : string.Empty, method.Name.Substring(4), args);
				else
					return string.Format("{0}/{1}{2} ({3})", targetName, method.IsStatic ? "static " : string.Empty, method.Name, args);
			}
		}

		private MethodInfo GetMethodInfo(SerializedProperty listenerTarget, SerializedProperty methodName, SerializedProperty arguments, SerializedProperty isStatic)
		{
			var desiredTypes = new Type[arguments.arraySize];
			for (int i = 0; i < arguments.arraySize; i++)
			{
			...
			return UnityEventBase2.GetValidMethodInfo(listenerTarget.objectReferenceValue, methodName.stringValue, desiredTypes, isStatic.boolValue);
		}
		...
		struct UnityEventFunction
		{
			...
			public void Clear()
			{
				...
				m_Listener.FindPropertyRelative(kIsStaticPath).boolValue = false;
